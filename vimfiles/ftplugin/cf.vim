""" DICTIONARY """
set dictionary=$VIM/vimfiles/CF.dict
set complete-=k complete+=k

""" AUTOMATIC FUNCTION FOLDING """
" This function is intended to be used as a foldexpr
" it will fold cold fusion functions. The settings that
" follow it will set it to be used and turn on expr based
" folding.
function! CFFunctionFold(line_number)
  if getline(a:line_number) =~ "<cffunction"
    return ">1"
  elseif getline(a:line_number) =~ "</cffunction"
    return "<1"
  else
    return "="
  endif
...
set foldexpr=CFFunctionFold(v:lnum)
set foldmethod=expr

""" This is the entire xml.vim file, there's gotta be a better way of """
""" injecting it into the cf file... in the meantime this works       """
